...
describe('RingCentral.http.ApiResponse', function() {
    ...
    function createResponse(status:number, statusText:string, body:any, headers?:any) {
        if (!headers) headers = jsonResponseHeaders;
        body = headers.trim() + '\n\n' + body;
        return RingCentral.sdk.http.ApiResponse.create(body, status, statusText);
    }
    ...
    describe('constructor tests', function() {
        ...
        it('calls the success callback for all individual parts that are parsed (including errors)', function() {
            ...
            expect(res.multipart().length).to.equal(3);

            //expect(res.data[0]).to.be.instanceOf(r.Response); //FIXME
            expect(res.multipart()[0].error()).to.be.equal(null);
            expect(res.multipart()[0].json().foo).to.be.equal('bar');
            expect(res.multipart()[0].response().status).to.be.equal(200);

            //expect(res.data[1]).to.be.instanceOf(r.Response); //FIXME
            expect(res.multipart()[1].error()).to.be.not.equal(null);

            //expect(res.data[2]).to.be.instanceOf(r.Response); //FIXME
            expect(res.multipart()[2].error()).to.be.equal(null);
            expect(res.multipart()[2].json().baz).to.be.equal('qux');
            expect(res.multipart()[2].response().status).to.be.equal(200);
