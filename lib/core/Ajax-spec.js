describe('RCSDK.core.Ajax', function() {
    ...
    describe('checkOptions method', function() {
        ...
        it('removes headers without value', function() {

            var ajax = rcsdk.getAjax()
                .setOptions({url: '/foo/bar', headers: {'content-type': false}})
                .checkOptions();

            expect('content-type' in ajax.options.headers).to.equal(false);
            expect(ajax.getRequestHeader('Content-Type')).to.equal(undefined);
    ...
    describe('parseResponse method', function() {
        ...
        it('parses empty response', function() {
            ...
            ajax.response = '';
            ajax.setResponseHeader('Content-Type', 'application/json');
            ajax.status = 200;

            var result = ajax.parseResponse();

            expect(result.error).to.equal(null);
            expect(ajax.data).to.equal('');
    ...
    describe('get/set headers', function() {

        it('sets headers and gets them disregarding the case', function() {

            var ajax = rcsdk.getAjax();

            ajax.setRequestHeader('content-type', 'foo');
            expect(ajax.getRequestHeader('Content-Type')).to.equal('foo');

            ajax.setRequestHeader('Content-Type', 'bar');
            expect(ajax.getRequestHeader('content-type')).to.equal('bar');

        });

        it('deletes the header if no value', function() {
