    ...
    describe('RCSDK.core.Utils', function () {
        ...
        describe('parseQueryString & queryStringify', function () {
            it('parses queryStrings', function () {
                expect(Utils.parseQueryString('foo=bar&bar=baz')).to.deep.equal({
                    foo: 'bar',
                    bar: 'baz'
                });
                expect(Utils.parseQueryString('foo=bar&foo=baz')).to.deep.equal({
                    foo: [
                        'bar',
                        'baz'
                    ]
        ...
        describe('extend', function () {
            it('applies properties of sources to target (only first level)', function () {
                var target = {
                        ...
                        foo: { id: 1 },
                        bar: { id: 1 }
                    }, source2 = { foo: { id: 2 } }, result = Utils.extend(target, source1, source2);
            ...
            it('applies all properties of sources to target in deep mode', function () {
                var target = {
                        ...
                        foo: { id: 1 },
                        bar: { id: 1 }
                    }, source2 = { foo: { id: 2 } }, result = Utils.extend(true, target, source1, source2);
                // make sure result is target
                expect(result).to.equal(target);
