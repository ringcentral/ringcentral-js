        ...
        describe('comparator', function () {
            it('by default provides comparator function which calls stringComparator with pre-bound extract functions', function () {
                var array = [
                        ...
                        { id: '2' },
                        { id: '1' }
                    ], sortFn = List.comparator({ sortBy: 'id' }), result = array.map(function (item) {
                        return item;
                    })    // we need a copy, original array is sorted in-place
.sort(sortFn);
            ...
            it('allows to provide custom sortBy property', function () {
                var array = [
                        ...
                        { name: '2' },
                        { name: '1' }
                    ], sortFn = List.comparator({ sortBy: 'name' }), result = array.map(function (item) {
                        return item;
                    })    // we need a copy, original array is sorted in-place
.sort(sortFn);
                expect(result[0]).to.equal(array[2]);
...
.sort(sortFn);
            ...
            it('provides options to custom extract and compare functions', function () {
                var array = [
                    ...
                    }, sortFn = List.comparator(options), result = array.map(function (item) {
                        return item;
                    })    // we need a copy, original array is sorted in-place
.sort(sortFn);
                expect(result[0]).to.equal(array[2]);
