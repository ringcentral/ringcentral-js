    ...
    describe('RCSDK.core.Platform', function () {
        ...
        describe('isAuthorized', function () {
            it('initiates refresh if not authorized', function (done) {
                ...
                platform.cancelAccessToken().isAuthorized().then(function () {
            ...
            it('waits for refresh to resolve from other tab', function (done) {
                ...
                platform.pause().cancelAccessToken().isAuthorized().then(function () {
        ...
        describe('apiCall', function () {
            it('refreshes token when token was expired', function (done) {
                ...
                mock.apiCall(path, {});
                expect(platform.getToken()).to.not.equal('ACCESS_TOKEN_FROM_REFRESH');
                platform.cancelAccessToken().on(platform.events.refreshSuccess, refreshSpy).apiCall({ url: path }).then(function (ajax) {
            ...
            it('tries to refresh the token if Platform returns 401 Unauthorized and re-executes the request', function (done) {
                var path = '/restapi/xxx', refreshSpy = spy(function () {
                    }), count = 0, response = { foo: 'bar' }, responseSpy = spy(function (ajax) {
                        count++;
                ...
                platform.on(platform.events.refreshSuccess, refreshSpy).apiCall({ url: path }).then(function (ajax) {
                    expect(refreshSpy).to.be.calledOnce;
            ...
            it('fails if ajax has status other than 2xx', function (done) {
                ...
                platform.apiCall({ url: path }).then(function (ajax) {
                    done(new Error('This should not be reached'));
                }).catch(function (e) {
        ...
        describe('refresh', function () {
            it('handles error in queued AJAX after unsuccessful refresh when token is killed', function (done) {
                var path = '/restapi/xxx', successSpy = spy(function () {
                    }), errorSpy = spy(function () {
                    });
                mock.tokenRefresh(true);
                mock.apiCall(path, {});
                platform.cancelAccessToken().on(platform.events.refreshSuccess, successSpy).on(platform.events.refreshError, errorSpy).apiCall({ url: path }).then(function () {
            ...
            it('handles subsequent refreshes', function (done) {
                mock.tokenRefresh();
                platform.refresh().then(function () {
                    return platform.refresh();
                }).then(function () {
                    return rcsdk.getContext().getPromise().all([
                        platform.refresh(),
                        platform.refresh()
                    ]);
                }).then(function () {
            ...
            it('returns error if response is malformed', function (done) {
                 ...
                 */
                    response: function (ajax) {
                        ajax.setStatus(240);
                        // This weird status were caught on client's machine
                        return {
                            'message': 'Wrong token',
        ...
        describe('refreshPolling', function () {
            ...
            it('polls the status of semaphor and resumes operation', function (done) {
                platform.pause().refreshPolling(null).then(function () {
                    done();
                }).catch(done);
                setTimeout(function () {
                    platform.resume();
            ...
            it('resolves with error if token is not valid after releaseTimeout', function (done) {
                platform.pause()    // resume() will not be called in this test
.cancelAccessToken().refreshPolling(null).then(function () {
                    done(new Error('This should not be reached'));
                }).catch(function (e) {
                    expect(e.message).to.equal('Automatic authentification timeout');
                    done();
