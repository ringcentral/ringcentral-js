describe('RCSDK.core.Platform', function() {
        ...
        it('refreshes token when token was expired', function(done) {
            ...
            platform
                ...
                .apiCall({
                    ...
                    expect(platform.getToken()).to.equal('ACCESS_TOKEN_FROM_REFRESH');
                    expect(ajax.request.getHeader('Authorization')).to.equal('bearer ACCESS_TOKEN_FROM_REFRESH');
                    done();
                }).catch(done);
    ...
    describe('refresh', function() {
        ...
        it('issues only one refresh request', function(done) {
            ...
            var spy2 = chai.spy(function(ajax) {
                return {
                    increment: ++increment,
                    constant: 1
                };
            ...
            Promise
                ...
                .then(function(res) {
                    ...
                    expect(spy).to.be.called.once();
                    expect(spy2).to.be.called.exactly(3);
                    done();
                })
                .catch(done);
