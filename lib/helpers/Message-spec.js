...
describe('RCSDK.helpers.Message', function() {
    ...
    describe('getAttachmentUrl & getAttachmentContentType', function() {
        ...
        it('gives a wrapped donwloadable url, empty string if not found', function() {
            ...
            expect(Message.getAttachmentUrl(message, 0)).to.equal('/restapi/v1.0/account/~/extension/~/message-store/1/attachment/---1---?_method=GET&access_token=' + platform.getToken());
            expect(Message.getAttachmentUrl(message, 1)).to.equal('/restapi/v1.0/account/~/extension/~/message-store/1/attachment/---2---?_method=GET&access_token=' + platform.getToken());
    ...
    describe('validate', function() {
        ...
        it('passes PagerMessage validation if values are correct', function() {
            ...
            var res = Message.validatePager({to: {extensionNumber: 'foo'}, from: {extensionNumber: 'foo'}, subject: 'foo'});
